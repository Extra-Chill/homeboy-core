...
mod tests {
    ...
    // ── runs query / drift integration tests ─────────────────────────────
    //
    // These exercise the schema-blind primitives end-to-end against the
    // real observation store. The fixtures imitate the artifact corpus an
    // ingestor would produce: each run has one JSON artifact attached.
    ...
    fn import_from_gh_actions_requires_gh_specific_arguments() {
        with_isolated_home(|_home| {
            let _xdg = XdgGuard::unset();
            // Without --component, --repo, --workflow, --artifact-glob,
            // the gh-actions branch must reject with a missing-argument error.
