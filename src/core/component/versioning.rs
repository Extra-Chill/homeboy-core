...
mod tests {
    ...
    fn test_validate_version_target_conflict_existing_pattern_new_pattern() {
        ...
        let _component_id = "";
        let result =
            validate_version_target_conflict(&existing, &new_file, &new_pattern, &_component_id);
        assert!(
            result.is_ok(),
            "expected Ok for: existing_pattern == new_pattern"
        );
    }
    ...
    fn test_parse_version_targets_if_let_some_p_pattern() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: if let Some(p) = pattern {{"
        );
    }
    ...
    fn test_parse_version_targets_let_some_p_pattern() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: let Some(p) = pattern"
        );
    }
    ...
    fn test_parse_version_targets_ok_parsed() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: Ok(parsed)"
        );
    }
