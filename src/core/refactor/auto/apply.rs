...
pub(crate) fn insert_import(content: &str, import_line: &str, language: &Language) -> String {
    ...
    if import_alias_collides(content, import_line, language) {
        return content.to_string();
    }
...
fn extract_import_alias(import_line: &str) -> Option<String> {
    ...
    // Handle `as Alias` — works for PHP, Rust, Python
    ...
    // PHP/Rust: `use Foo\Bar\Baz` or `use foo::bar::Baz` → `Baz`
    ...
    // Skip brace-grouped imports like `use foo::{A, B};` — these have
    // multiple aliases and aren't generated by the refactor pipeline.
    if path.contains('{') {
        return None;
    }
...
/// existing import in the file. Returns `true` if the new import's short name
/// (alias) is already claimed by a different import statement.
    ...
    for line in content.lines() {
        ...
        if let Some(existing_alias) = extract_import_alias(trimmed) {
            if existing_alias == candidate_alias {
                // Same alias — but only a collision if the full import is different.
                // If they're the same import, `import_already_present` handles it.
...
";
    ...
    #[test]
    fn extract_import_alias_php_fqn() {
        assert_eq!(
            extract_import_alias("use DataMachine\\Core\\OAuth\\OAuth1Handler;"),
            Some("OAuth1Handler".to_string())
        );
    }

    #[test]
    fn extract_import_alias_php_with_as() {
        assert_eq!(
            extract_import_alias("use DataMachine\\Core\\OAuth\\OAuth1Handler as Handler;"),
            Some("Handler".to_string())
        );
    }

    #[test]
    fn extract_import_alias_rust_path() {
        assert_eq!(
            extract_import_alias("use crate::core::refactor::OAuth1Handler;"),
            Some("OAuth1Handler".to_string())
        );
    }

    #[test]
    fn extract_import_alias_brace_group_returns_none() {
        assert_eq!(
            extract_import_alias("use std::collections::{HashMap, HashSet};"),
            None
        );
    }

    #[test]
    fn insert_import_skips_php_alias_collision() {
        let content = "<?php\n\nnamespace DataMachine\\Core\\OAuth;\n\nuse DataMachine\\Other\\OAuth1Handler;\n\nclass BaseOAuth2Provider {}\n";
        let result = insert_import(
            content,
        ...
        assert_eq!(
            result, content,
            "Should skip import that would collide on alias OAuth1Handler"
        );
    }

    #[test]
    fn insert_import_allows_different_alias_php() {
        let content = "<?php\n\nnamespace DataMachine\\Core\\OAuth;\n\nuse DataMachine\\Other\\OAuth1Handler;\n\nclass BaseOAuth2Provider {}\n";
        let result = insert_import(
            content,
            "use DataMachine\\Abilities\\Traits\\HasOAuth2Handler;",
            &Language::Php,
        );
        assert!(
            result.contains("HasOAuth2Handler"),
            "Should insert import with a different alias"
        );
    }

    #[test]
    fn insert_import_skips_rust_alias_collision() {
        let content = "use other_crate::Config;\n\npub fn run() {}\n";
        let result = insert_import(content, "use crate::settings::Config;", &Language::Rust);
        assert_eq!(
            result, content,
            "Should skip import that would collide on alias Config"
        );
    }

    #[test]
    fn import_alias_collides_same_import_not_collision() {
        let content = "use DataMachine\\Core\\OAuth\\OAuth1Handler;\n";
        assert!(
            !import_alias_collides(
                content,
                "use DataMachine\\Core\\OAuth\\OAuth1Handler;",
                &Language::Php
            ),
