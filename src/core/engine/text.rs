...
mod tests {
    ...
    fn test_extract_all_default_path() {
        let content = "";
        let pattern = "";
        let _result = extract_all(&content, &pattern);
    }
    ...
    fn test_extract_all_some_matches() {
        ...
        assert!(result.is_some(), "expected Some for: Some(matches)");
    }
    ...
    fn test_split_whitespace_parts_len_min_parts() {
        ...
        assert!(result.is_some(), "expected Some for: parts.len() >= min_parts");
    }
