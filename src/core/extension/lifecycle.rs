...
mod tests {
    ...
    fn test_check_update_available_extension_dir_exists_is_extension_linked_extension_id() {
        ...
        assert!(
            result.is_none(),
            "expected None for: !extension_dir.exists() || is_extension_linked(extension_id)"
        );
    }
    ...
    fn test_check_update_available_extension_dir_join_git_exists() {
        ...
        assert!(
            result.is_none(),
            "expected None for: !extension_dir.join(\".git\").exists()"
        );
    }
    ...
    fn test_read_source_revision_extension_dir_exists() {
        ...
        assert!(
            result.is_none(),
            "expected None for: !extension_dir.exists()"
        );
    }
    ...
    fn test_read_source_revision_extension_dir_exists_2() {
        ...
        assert!(
            result.is_some(),
            "expected Some for: !extension_dir.exists()"
        );
    }
    ...
    fn test_read_source_revision_let_some_rev_get_short_head_revision_extension_dir() {
        ...
        assert!(
            result.is_some(),
            "expected Some for: let Some(rev) = get_short_head_revision(&extension_dir)"
        );
    }
