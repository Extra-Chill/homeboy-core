...
mod block_syntax;
mod convenience_helpers_for_feature_consumers;
mod extraction_apply_grammar_patterns_to_get_symbols;
mod grammar_definition_loaded_from_extension_toml_json;
mod grammar_loading;
mod structural_context;
mod structural_parser_context_aware_iteration_over_source_text;
mod symbol;
mod types;

pub use block_syntax::*;
pub use convenience_helpers_for_feature_consumers::*;
pub use extraction_apply_grammar_patterns_to_get_symbols::*;
pub use grammar_definition_loaded_from_extension_toml_json::*;
pub use grammar_loading::*;
pub use structural_context::*;
pub use structural_parser_context_aware_iteration_over_source_text::*;
pub use symbol::*;
pub use types::*;
...
"#;
    ...
    fn test_extract_ok_r_r() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: Ok(r) => r,"
        );
    }
    ...
    fn test_extract_err_continue_skip_invalid_patterns() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: Err(_) => continue, // Skip invalid patterns"
        );
    }
    ...
    fn test_extract_if_let_some_caps_re_captures_ctx_line_text() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: if let Some(caps) = re.captures(ctx_line.text) {{"
        );
    }
    ...
    fn test_extract_let_some_caps_re_captures_ctx_line_text() {
        ...
        assert!(
            !result.is_empty(),
            "expected non-empty collection for: let Some(caps) = re.captures(ctx_line.text)"
        );
    }
