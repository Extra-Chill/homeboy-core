...
use crate::rig::{discover_rigs, install, list_sources, read_source_metadata, remove_source};
use std::fs;
use std::path::Path;
...
#[test]
fn sources_list_groups_local_linked_package_metadata() {
    let _home = HomeGuard::new();
    let package = tempfile::tempdir().expect("package");
    write_rig(package.path(), "alpha", &minimal_rig("alpha"));
    write_rig(package.path(), "beta", &minimal_rig("beta"));
...
#[test]
fn sources_remove_deletes_multi_rig_configs_and_metadata_only() {
    let _home = HomeGuard::new();
    let package = tempfile::tempdir().expect("package");
    write_rig(package.path(), "alpha", &minimal_rig("alpha"));
    write_rig(package.path(), "beta", &minimal_rig("beta"));
...
#[test]
fn sources_remove_skips_config_that_no_longer_matches_metadata() {
    let _home = HomeGuard::new();
    let package = tempfile::tempdir().expect("package");
    write_rig(package.path(), "alpha", &minimal_rig("alpha"));
